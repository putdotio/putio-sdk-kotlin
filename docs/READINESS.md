...
- `./gradlew verify` now enforces compile, unit-test, jar, and coverage guardrails
...
| Unit verification | `good` | request shaping, parsing, unknown-value preservation, typed API errors, and localized recovery guidance have deterministic coverage across current domains |
...
1. deepen deterministic coverage for more conditional payload branches and transport failure paths
