...
class PutioTransportTest {
    @Test
    fun `get with auth none omits authorization header`() = withServer { server ->
    ...
    @Test
    fun `config token auth requires an access token`() = withServer { server ->
    ...
    @Test
    fun `transport wraps undecodable success payloads as serialization errors`() = withServer { server ->
        server.enqueue(MockResponse.Builder().body("""{"status":"OK","unexpected":true}""").build())

        val transport = newTransport(server, accessToken = "secret-token")

        val error = assertFailsWith<PutioSerializationException> {
            kotlinx.coroutines.runBlocking {
                transport.get(
                    path = "/files/list",
    ...
    @Test
    fun `transport falls back to http status when error payload is not json`() = withServer { server ->
        server.enqueue(
